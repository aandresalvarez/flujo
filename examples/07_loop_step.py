...
loop_body_pipeline = Pipeline.from_step(
    Step("EditAndReview", agent=cast(Any, edit_and_review_agent))
)
...
full_pipeline = Step(
    "GenerateInitialDraft", agent=cast(Any, generate_text_agent)
) >> loop_step

runner = Flujo(full_pipeline)

print("🚀 Starting iterative refinement pipeline...\n")
result = asyncio.run(runner.run_async("Write a sentence about Python."))
