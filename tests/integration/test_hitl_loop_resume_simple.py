...
async def test_hitl_in_loop_no_nesting_simple():
    ...
    agent_step = Step(
        name="agent",
        agent=StubAgent(["agent_output"])
    )
    
    hitl_step = HumanInTheLoopStep(
        name="hitl",
        message_for_user="Respond please"
    )
...
async def test_hitl_in_loop_agent_output_captured_simple():
    ...
    if hasattr(ctx1, "steps") and "agent" in ctx1.steps:
        ...
        assert agent_output.get("count") == 1, f"Agent output should show call 1, got: {agent_output}"
...
async def test_hitl_in_loop_multiple_iterations_simple():
    ...
    # Create steps
    agent_step = Step(name="increment", agent=tracking_agent(), updates_context=True, sink_to="counter")
    hitl_step = HumanInTheLoopStep(name="hitl", message_for_user="Continue?")
    ...
    assert result.final_pipeline_context.scratchpad.get("status") == "paused", "Should pause in iteration 1"
