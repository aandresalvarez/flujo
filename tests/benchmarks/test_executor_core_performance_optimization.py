...
class TestExecutorCorePerformance:
    ...
    async def test_executor_core_execution_performance(self, executor_core, perf_benchmark):
        ...
        assert stats['mean'] < threshold, f"Mean execution time {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
        assert stats['p95'] < threshold * 2, f"P95 execution time {stats['p95']:.6f}s exceeds threshold {threshold * 2:.6f}s"
    ...
    async def test_executor_core_memory_usage(self, executor_core, perf_benchmark):
        ...
        assert memory_increase < threshold, f"Memory increase {memory_increase:.2f}MB exceeds threshold {threshold:.2f}MB"
    ...
    async def test_executor_core_concurrent_execution(self, executor_core, perf_benchmark):
        ...
        async def concurrent_execution():
            tasks = [
                executor_core.execute(step, data)
        ...
        assert stats['mean'] < threshold, f"Concurrent execution time {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
    ...
    async def test_executor_core_cache_performance(self, executor_core, perf_benchmark):
        ...
        # Cache should provide some speedup (minimal expectation for very fast operations)
        # For very fast operations, even small improvements are valuable
        assert cache_speedup >= 1.0, f"Cache speedup {cache_speedup:.2f}x should be at least 1.0x (no regression)"
    ...
    @pytest.mark.asyncio
    @pytest.mark.benchmark
    async def test_executor_core_context_handling_performance(self, executor_core, perf_benchmark, mock_context):
        ...
        assert overhead_percentage < max_overhead_percentage, f"Context overhead {overhead_percentage:.1f}% exceeds threshold {max_overhead_percentage:.1f}%"
...
class TestComponentPerformance:
    ...
    async def test_agent_runner_performance(self, executor_core, perf_benchmark):
        ...
        assert stats['mean'] < threshold, f"Agent runner performance {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
    ...
    async def test_processor_pipeline_performance(self, executor_core, perf_benchmark):
        ...
        threshold = get_performance_threshold(0.01)  # 10ms base threshold
        assert stats['mean'] < threshold, f"Processor pipeline performance {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
    ...
    async def test_validator_runner_performance(self, executor_core, perf_benchmark):
        ...
        threshold = get_performance_threshold(0.01)  # 10ms base threshold
        assert stats['mean'] < threshold, f"Validator runner performance {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
    ...
    async def test_plugin_runner_performance(self, executor_core, perf_benchmark):
        ...
        threshold = get_performance_threshold(0.01)  # 10ms base threshold
        assert stats['mean'] < threshold, f"Plugin runner performance {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
...
class TestMemoryManagement:
    ...
    async def test_garbage_collection_impact(self, executor_core, perf_benchmark):
        ...
        try:
            ...
            assert impact_percentage < max_impact_percentage, f"GC impact {impact_percentage:.1f}% exceeds threshold {max_impact_percentage:.1f}%"
    ...
    async def test_memory_leak_prevention(self, executor_core, perf_benchmark):
        ...
        if len(memory_samples) > 2:
            ...
            assert growth_per_iteration < max_growth_per_iteration, f"Memory growth {growth_per_iteration:.3f}MB per iteration exceeds threshold {max_growth_per_iteration:.3f}MB"
    ...
    async def test_object_pooling_performance(self, executor_core, perf_benchmark):
        ...
        threshold = get_performance_threshold(0.01)  # 10ms base threshold
        assert stats['mean'] < threshold, f"Object pooling baseline {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
...
class TestAdvancedComponentPerformance:
    ...
    async def test_serialization_performance(self, executor_core, perf_benchmark):
        ...
        for size_name, data in data_sizes:
            ...
            assert stats['mean'] < threshold, f"Serialization performance for {size_name} data {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
    ...
    async def test_hash_computation_performance(self, executor_core, perf_benchmark):
        ...
        for i, data in enumerate(test_data):
            ...
            threshold = get_performance_threshold(0.01)  # 10ms base threshold
            assert stats['mean'] < threshold, f"Hash computation performance for test {i+1} {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
    ...
    async def test_cache_key_generation_performance(self, executor_core, perf_benchmark):
        ...
        test_scenarios = [
            ...
            {"data": {"complex": {"nested": "data"}}, "context": {"ctx": "test"}, "resources": None},
            {"data": {"large": "x" * 1000}, "context": {"large_ctx": "y" * 500}, "resources": {"res": "z" * 200}}
        ]
        ...
        for i, scenario in enumerate(test_scenarios):
            ...
            assert stats['mean'] < threshold, f"Cache key generation performance for scenario {i+1} {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
    ...
    async def test_context_copying_performance(self, executor_core, perf_benchmark):
        ...
        contexts = [
            ...
            {"large": {f"key_{i}": {"nested": f"value_{i}", "list": list(range(10))} for i in range(50)}}
        ]
        ...
        for size_name, context in contexts:
            ...
            threshold = get_performance_threshold(0.02)  # 20ms base threshold
            assert stats['mean'] < threshold, f"Context copying performance for {size_name} context {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
...
class TestConcurrencyPerformance:
    ...
    async def test_semaphore_contention_performance(self, perf_benchmark):
        ...
        for limit in concurrency_limits:
            ...
            assert stats['mean'] < threshold, f"Semaphore contention performance with limit {limit} {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
    ...
    async def test_task_scheduling_performance(self, executor_core, perf_benchmark):
        ...
        for pattern in task_patterns:
            ...
            assert stats['mean'] < threshold, f"Task scheduling performance for {pattern['name']} pattern {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
    ...
    async def test_resource_contention_performance(self, executor_core, perf_benchmark):
        ...
        threshold = get_performance_threshold(1.0)  # 1s base threshold
        assert stats['mean'] < threshold, f"Resource contention performance {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"
...
class TestScalabilityBenchmarks:
    ...
    async def test_linear_scaling_performance(self, perf_benchmark):
        ...
        for load in loads:
            executor = ExecutorCore(
                enable_cache=True,
                concurrency_limit=max(4, load // 2)
            )
        ...
        if len(performance_data) >= 2:
            ...
            assert scaling_factor < max_scaling_factor, f"Scaling factor {scaling_factor:.2f}x exceeds threshold {max_scaling_factor:.2f}x"
    ...
    async def test_memory_scaling_performance(self, perf_benchmark):
        ...
        for load in loads:
            ...
            print(f"Total memory increase: {final_memory['rss_mb'] - initial_memory['rss_mb']:.2f} MB")
            
            # Memory per execution should be reasonable
            max_memory_per_execution = get_performance_threshold(0.5)  # 0.5MB per execution
            assert memory_per_execution < max_memory_per_execution, f"Memory per execution {memory_per_execution:.3f}MB exceeds threshold {max_memory_per_execution:.3f}MB"
    ...
    async def test_cache_scaling_performance(self, perf_benchmark):
        ...
        for cache_size in cache_sizes:
            ...
            assert stats['mean'] < threshold, f"Cache hit performance with size {cache_size} {stats['mean']:.6f}s exceeds threshold {threshold:.6f}s"