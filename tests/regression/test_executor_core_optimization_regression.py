...
from flujo.application.core.ultra_executor import (
    ExecutorCore, OptimizedExecutorCore, OptimizationConfig,
...
class TestOptimizationFunctionalityPreservation:
    ...
    @pytest.mark.asyncio
    async def test_basic_step_execution_preserved(self, standard_executor, optimized_executor, basic_step, test_data):
    ...
    @pytest.mark.asyncio
    async def test_context_passing_preserved(self, standard_executor, optimized_executor, test_data):
        ...
        standard_result = await standard_executor.execute(context_step, test_data, context=test_context)
        optimized_result = await optimized_executor.execute(context_step, test_data, context=test_context)
    ...
    @pytest.mark.asyncio
    async def test_caching_behavior_preserved(self, standard_executor, optimized_executor, basic_step, test_data):
    ...
    @pytest.mark.asyncio
    async def test_usage_limits_preserved(self, standard_executor, optimized_executor, basic_step, test_data):
...
class TestOptimizationBackwardCompatibility:
    ...
    def test_configuration_serialization(self):
        ...
        assert restored_config.enable_context_optimization == original_config.enable_context_optimization
...
class TestOptimizationPerformanceRegression:
    ...
    def performance_step(self):
        ...
        return create_test_step("performance_step", ["performance_result"] * 50)  # Extra outputs for multiple uses
    ...
    async def test_execution_time_regression(self, performance_step, performance_data):
        ...
        # Allow up to 2.5x slower for simple operations in test environment
        assert optimized_time < standard_time * 2.5, (
            f"Optimized executor too slow: {optimized_time:.4f}s vs {standard_time:.4f}s"
        )
        
        print(f"Performance comparison: Standard={standard_time:.6f}s, Optimized={optimized_time:.6f}s")
