...
class Flujo(Generic[RunnerInT, RunnerOutT, ContextT]):
    ...
    async def resume_async(
        ...
        async def _resume_impl() -> PipelineResult[ContextT]:
            ...
            if isinstance(paused_step, HumanInTheLoopStep) and paused_step.input_schema is not None:
                current_input = paused_step.input_schema.model_validate(human_input)
            else:
                current_input = human_input
            ...
            if hasattr(paused_step, "sink_to") and paused_step.sink_to and ctx is not None:
                try:
                    ...
                    set_nested_context_field(ctx, paused_step.sink_to, current_input)
                except Exception:
                    pass
            ...
            exec_manager.set_final_context(paused_result, cast(Optional[ContextT], ctx))
            try:
                await self._dispatch_hook(
                    "post_run",
