...
def _accepts_param(func: Callable[..., Any], param: str) -> Optional[bool]:
    ...
    using a :class:`~weakref.WeakKeyDictionary` keyed by the callable object to
    avoid stale cache entries when bound method objects are garbage collected.
    """
    if func in _accepts_param_cache and param in _accepts_param_cache[func]:
        return _accepts_param_cache[func][param]
    ...
    if func not in _accepts_param_cache:
        _accepts_param_cache[func] = {}
    _accepts_param_cache[func][param] = result
    return result
...
class Flujo(Generic[RunnerInT, RunnerOutT]):
    ...
    async def _run_step(
        ...
        if isinstance(step, LoopStep):
            ...
            message = (
                step.message_for_user
                if step.message_for_user is not None
                else str(data)
            )
    ...
    def run(
        ...
        *,
        ...
        return asyncio.run(
            self.run_async(initial_input, initial_context_data=initial_context_data)
        )
