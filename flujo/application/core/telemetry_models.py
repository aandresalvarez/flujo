...
from typing import Any, Dict, List, Optional, Set, Callable, Union, Tuple, Generic, TypeVar
...
class ExecutionStats:
    ...
    min_execution_time_ns: int = float('inf')
    ...
    recent_cache_hit_rates: deque = field(default_factory=lambda: deque(maxlen=100))
    ...
    def update_with_metrics(self, metrics: PerformanceMetrics, status: ExecutionStatus) -> None:
        ...
        if status == ExecutionStatus.SUCCESS:
            ...
            self.min_execution_time_ns = min(self.min_execution_time_ns, metrics.execution_time_ns)
        ...
        self.max_concurrent_executions = max(self.max_concurrent_executions, metrics.concurrent_executions)
    ...
    def get_trend_analysis(self) -> Dict[str, Any]:
        ...
        if len(self.recent_execution_times) >= 10:
            times = list(self.recent_execution_times)
        ...
        if len(self.recent_memory_usage) >= 10:
            memory = list(self.recent_memory_usage)
...
class MetricAggregator:
    ...
    def __init__(self, window_size_seconds: float = 300.0):  # 5 minutes default
        ...
        self._metrics_buffer: deque = deque()
        self._lock = RLock()
    ...
    def get_aggregated_metrics(self) -> AggregatedMetrics:
        ...
        with self._lock:
            ...
            if window_metrics:
                ...
                aggregated.total_execution_time_ns = sum(m.execution_time_ns for m in window_metrics)
                aggregated.total_memory_allocated = sum(m.memory_allocated_bytes for m in window_metrics)
                aggregated.total_cache_operations = sum(m.cache_hits + m.cache_misses for m in window_metrics)
                
                # Statistical summaries
                execution_times = [m.execution_time_ns for m in window_metrics]
                memory_usage = [m.memory_peak_bytes for m in window_metrics]
                cache_hit_rates = [
                    m.cache_hits / max(m.cache_hits + m.cache_misses, 1) 
