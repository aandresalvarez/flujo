...
class ErrorClassifier:
    ...
    def __init__(self) -> None:
        ...
        self._category_patterns = {
            ErrorCategory.NETWORK: ["connection", "network", "socket", "dns", "ssl", "tls"],
...
class OptimizedErrorHandler:
        ...
        **kwargs: Any,
        ...
        try:
            ...
            if self.enable_stats and self._stats is not None:
                ...
                self._stats.recovery_action_counts[recovery_result.action_taken] = (
                    self._stats.recovery_action_counts.get(recovery_result.action_taken, 0) + 1
                )

                if recovery_result.strategy_used:
                    self._stats.strategy_usage_counts[recovery_result.strategy_used] = (
                        self._stats.strategy_usage_counts.get(recovery_result.strategy_used, 0) + 1
                    )

                self._stats.total_recovery_time_ms += recovery_result.recovery_time_ms
    ...
    def _cache_recovery_result(
        self, error_context: ErrorContext, result: ErrorRecoveryResult
    ) -> None:
        ...
        if strategy.custom_recovery_func:
            try:
                recovered_value = await self._execute_custom_recovery(
                    strategy.custom_recovery_func, error_context
                )
    ...
    async def _execute_custom_recovery(
        self, recovery_func: Callable[[ErrorContext], Any], error_context: ErrorContext
    ) -> Any:
    ...
    async def _execute_standard_recovery(
        self, error_context: ErrorContext, strategy: RecoveryStrategy, start_time: float
    ) -> ErrorRecoveryResult:
        """Execute standard recovery actions."""
    ...
    def _record_telemetry(self, error_context: ErrorContext, result: ErrorRecoveryResult) -> None:
        ...
        self._telemetry.record_histogram(
            ...
            tags={"action": result.action_taken.value, "success": str(result.success).lower()},
        )
        ...
        with self._lock:
            analysis = {
                ...
                "top_error_types": dict(
                    sorted(self._stats.error_type_counts.items(), key=lambda x: x[1], reverse=True)[
                        :10
                    ]
                ),
                ...
                "strategy_effectiveness": {
                    strategy: count for strategy, count in self._stats.strategy_usage_counts.items()
                },
            }
    ...
    def suggest_optimizations(self) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            if self._stats.error_type_counts:
                most_common_error = max(self._stats.error_type_counts.items(), key=lambda x: x[1])
                ...
                try:
                    ...
                    if attempt == max_retries:
                        ...
                        result = await handler.handle_error(
                            error, step_name=step_name or func.__name__, attempt_number=attempt + 1
                        )
