...
class CircuitBreakerStats:
    ...
    # Recent performance
    recent_calls: deque[CallResult] = field(default_factory=lambda: deque(maxlen=100))
...
class CircuitBreaker:
    ...
    def __init__(self, name: str, config: Optional[CircuitBreakerConfig] = None):
        ...
        self._failure_times: deque[float] = deque(maxlen=100)
        self._success_times: deque[float] = deque(maxlen=100)
    ...
    async def call(
        ...
        **kwargs: Any,
        ...
        if not self._should_allow_call():
            ...
            if self._telemetry:
                self._telemetry.increment_counter(
                    "circuit_breaker.calls_rejected", tags={"circuit_name": self.name}
                )
        ...
        try:
            ...
            is_custom_failure = (
                self.config.custom_failure_predicate
                and self.config.custom_failure_predicate(result)
            )

            if is_slow:
                call_result = CallResult(
                    success=False, duration_ms=duration_ms, failure_type=FailureType.SLOW_RESPONSE
                )
            elif is_custom_failure:
                call_result = CallResult(
                    success=False, duration_ms=duration_ms, failure_type=FailureType.CUSTOM
                )
            else:
            ...
            duration_ms = (time.perf_counter() - start_time) * 1000
            call_result = CallResult(
                success=False, duration_ms=duration_ms, failure_type=FailureType.TIMEOUT, error=e
            )
            ...
            duration_ms = (time.perf_counter() - start_time) * 1000
            call_result = CallResult(
                success=False, duration_ms=duration_ms, failure_type=FailureType.EXCEPTION, error=e
            )
    ...
    def _record_call_result(self, result: CallResult) -> None:
        ...
        with self._lock:
            ...
            while (
                ...
                and current_time - self._failure_window[0] > self.config.failure_window_seconds
            ):
        ...
        if self._telemetry:
            ...
            self._telemetry.record_histogram(
                ...
                result.duration_ms,
                tags={"circuit_name": self.name, "success": str(result.success).lower()},
            )
    ...
    def _should_open_circuit(self) -> bool:
        ...
        if recent_calls > 0:
            recent_failures = sum(
                1 for call in list(self._call_history)[-recent_calls:] if not call.success
            )
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
            return await with_circuit_breaker(
                name, func, *args, config=config, timeout_seconds=timeout_seconds, **kwargs
            )
        ...
        def sync_wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                is_custom_failure = (
                    breaker.config.custom_failure_predicate
                    and breaker.config.custom_failure_predicate(result)
                )
