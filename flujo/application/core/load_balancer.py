...
from typing import Any, Dict, List, Optional, Set, Callable, Tuple, Awaitable
...
class TaskQueue:
    ...
    def __init__(self) -> None:
        self._priority_queues: Dict[TaskPriority, List[Tuple[int, float, Task]]] = {
    ...
    def complete_task(self, task_id: str) -> None:
        ...
        with self._lock:
            if task_id in self._task_map:
                ...
                # task = self._task_map.pop(task_id)  # Unused variable
                    ...
                    if dependent_id in self._task_map:
                        ...
                        if dependent_task.is_ready:
                            heapq.heappush(
                                ...
                                (
                                    -dependent_task.priority.value,
                                    dependent_task.created_at,
                                    dependent_task,
                                ),
                            )
    ...
    def get_queue_stats(self) -> JSONObject:
        ...
        with self._lock:
            return {
                ...
                "queued_by_priority": {
                    priority.name: len(queue) for priority, queue in self._priority_queues.items()
                },
                "pending_dependencies": len(
                    [task for task in self._task_map.values() if not task.is_ready]
                ),
            }
...
class LoadBalancer:
    ...
    def add_worker(
        ...
        with self._lock:
            worker = Worker(
                worker_id=worker_id, capacity=capacity, resource_capacity=resource_capacity or {}
            )
    ...
    async def submit_task(self, task: Task) -> str:
        ...
        if self._telemetry:
            self._telemetry.increment_counter(
                "load_balancer.tasks_submitted", tags={"priority": task.priority.name}
            )
        ...
        try:
            ...
            if self._telemetry:
                self._telemetry.increment_counter(
                    ...
                    tags={"worker_id": worker.worker_id, "error_type": type(e).__name__},
                )
    ...
    def _select_worker(self, task: Task) -> Optional[Worker]:
        ...
        available_workers = [
            worker for worker in self._workers.values() if worker.can_handle_task(task)
        ]
