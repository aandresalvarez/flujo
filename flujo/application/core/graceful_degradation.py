...
from .adaptive_resource_manager import (
    ResourceType, PressureLevel, get_global_adaptive_resource_manager
)
...
class DegradationController:
    ...
    def __init__(
        ...
        self._degradation_history: deque = deque(maxlen=1000)
        ...
        try:
            ...
            if self._performance_monitor:
                ...
                if 'application_metrics' in perf_summary:
                    ...
                    if 'executor.errors' in app_metrics and 'executor.executions_total' in app_metrics:
    ...
    def _calculate_required_degradation_level(self, metrics: SystemHealthMetrics) -> DegradationLevel:
        ...
        for level in [DegradationLevel.CRITICAL, DegradationLevel.SEVERE, 
        ...
        if target_level.value > old_level.value:  # Increasing degradation
            ...
            for feature in self._features.values():
                if (feature.enabled and 
                    feature.disable_at_level.value <= target_level.value):
            ...
            for feature in self._features.values():
                if (not feature.enabled and 
                    feature.disable_at_level.value > target_level.value and
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                'thresholds': {
                    'degradation': {level.value: threshold for level, threshold in self._degradation_thresholds.items()},
                    'recovery': {level.value: threshold for level, threshold in self._recovery_thresholds.items()}
                }
            }
...
def register_feature(
    ...
    controller.register_feature(
        name, category, cpu_impact, memory_impact, 0.0, 
        disable_at_level, disable_callback, enable_callback
    )
