...
class StepAnalyzer:
    ...
    def analyze_step(self, step: Any) -> StepAnalysis:
        ...
        with self._lock:
            ...
            estimated_duration = self._estimate_duration(complexity, has_processors, has_validators, has_plugins)
    ...
    def _determine_complexity(self, step: Any) -> StepComplexity:
        ...
        if hasattr(step, 'processors') and (
            getattr(step.processors, 'prompt_processors', []) or 
        ...
        try:
            ...
            if executable_func:
                ...
                try:
                    ...
                    # Fallback analysis
                    analysis['accepts_context'] = hasattr(agent, 'run') and 'context' in str(inspect.signature(agent.run))
                    analysis['accepts_resources'] = hasattr(agent, 'run') and 'resources' in str(inspect.signature(agent.run))
    ...
    def _has_processors(self, step: Any) -> bool:
        ...
        return bool(
            getattr(processors, 'prompt_processors', []) or 
            getattr(processors, 'output_processors', [])
        )
    ...
    def _can_use_fast_path(
        ...
        return (
            complexity == StepComplexity.SIMPLE and
            not has_processors and
            not has_validators and
            not has_plugins and
            not has_fallback
        )
    ...
    def _requires_deep_context_copy(self, step: Any) -> bool:
        ...
        return (
            hasattr(step, 'plugins') and step.plugins or
            hasattr(step, 'processors') and self._has_processors(step)
        )
...
class OptimizedStepExecutor:
    ...
    def __init__(
        ...
        self._result_pool: deque = deque(maxlen=100)  # Pool of reusable StepResult objects
        ...
        **kwargs: Any
        ...
        try:
            ...
            if self.enable_fast_path and analysis.can_use_fast_path:
                result = await self._execute_fast_path(step, data, context, resources, analysis, **kwargs)
            else:
                result = await self._execute_standard_path(step, data, context, resources, analysis, **kwargs)
    ...
    async def _return_result_object(self, result: StepResult) -> None:
        ...
        if len(self._result_pool) < self._result_pool.maxlen:
    ...
    def _update_execution_success_stats(
        ...
        with self._lock:
            ...
            self._global_stats.min_duration_ns = min(self._global_stats.min_duration_ns, execution_time_ns)
            self._global_stats.max_duration_ns = max(self._global_stats.max_duration_ns, execution_time_ns)
    ...
    def _update_execution_failure_stats(
        ...
        with self._lock:
            ...
            self._global_stats.total_duration_ns += execution_time_ns
            self._global_stats.error_types[error_type] = self._global_stats.error_types.get(error_type, 0) + 1
