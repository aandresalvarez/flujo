...
class StrategyStats:
    ...
    def update_attempt(
        self, success: bool, recovery_time_ms: float, error_type: str, category: str
    ) -> None:
...
class BaseRecoveryStrategy(ABC):
    ...
    def _record_attempt(
        self, success: bool, recovery_time_ms: float, error_context: ErrorContext
    ) -> None:
        """Record recovery attempt in statistics."""
        with self._lock:
            self._stats.update_attempt(
                success, recovery_time_ms, error_context.error_type, error_context.category.value
            )
    ...
    def __init__(
        ...
        if self.config.enable_circuit_breaker:
            ...
            cb_config = CircuitBreakerConfig(
                ...
                recovery_timeout_seconds=int(self.config.max_delay_seconds),
            )
...
class FallbackStrategy(BaseRecoveryStrategy):
    ...
    def should_apply(self, error_context: ErrorContext) -> bool:
        ...
        has_fallback = (
            self.config.fallback_value is not None
            or self.config.fallback_function is not None
            or self.config.fallback_exception_handler is not None
        )
        ...
        try:
            ...
            # Try exception handler
                ...
                if asyncio.iscoroutinefunction(self.config.fallback_exception_handler):
                    fallback_value = await self.config.fallback_exception_handler(
                        error_context.error
                    )
                else:
...
class CompositeStrategy(BaseRecoveryStrategy):
    ...
    def __init__(
        ...
        self._strategy_selector: Optional[
            Callable[[ErrorContext, List[BaseRecoveryStrategy]], BaseRecoveryStrategy]
        ] = None
    ...
    def set_strategy_selector(
        self, selector: Callable[[ErrorContext, List[BaseRecoveryStrategy]], BaseRecoveryStrategy]
    ) -> None:
    ...
    async def execute_recovery(
        ...
        **kwargs: Any,
        ...
        applicable_strategies = [
            strategy for strategy in self.strategies if strategy.should_apply(error_context)
        ]
...
class StrategyRegistry:
    ...
    def find_applicable_strategies(self, error_context: ErrorContext) -> List[BaseRecoveryStrategy]:
    ...
    def get_best_strategy(self, error_context: ErrorContext) -> Optional[BaseRecoveryStrategy]:
    ...
    def _register_default_strategies(self) -> None:
        ...
        validation_fallback = FallbackStrategy(
            "validation_fallback",
            FallbackConfig(fallback_value=None, fallback_on_exceptions=[ValueError, TypeError]),
        )
        ...
        external_circuit_breaker = CircuitBreakerStrategy(
            ...
            CircuitBreakerConfig(
                failure_threshold=5, failure_rate_threshold=0.5, recovery_timeout_seconds=30
            ),
        )
        ...
        comprehensive_strategy = CompositeStrategy(
            "comprehensive_recovery", [network_retry, validation_fallback, external_circuit_breaker]
        )
