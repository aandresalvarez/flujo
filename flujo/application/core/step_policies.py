...
import time
from flujo.domain.dsl.step import MergeStrategy, BranchFailureStrategy, HumanInTheLoopStep
...
class DefaultLoopStepExecutor:
    ...
    async def execute(
        ...
        for iteration_count in range(1, max_loops + 1):
            ...
            # These are used later in usage limit exception handling (lines 2464, 2467, etc.)
            prev_current_context = current_context  # Used in usage limit exception handling
            prev_cumulative_cost = cumulative_cost  # Used in usage limit exception handling
            prev_cumulative_tokens = cumulative_tokens  # Used in usage limit exception handling
            iteration_context = (
