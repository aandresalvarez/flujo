    ...
    def validate_value(self, value: Any) -> bool:
        ...
        try:
            if isinstance(self.min_value, (int, float)) and isinstance(
                ...
                return bool(self.min_value <= value <= self.max_value)
            return True
...
class OptimizationParameterTuner:
    ...
    def get_system_optimized_defaults(self) -> JSONObject:
        ...
        defaults["object_pool_max_pools"] = int(min(200, cpu_count * 25))
        ...
        defaults["context_ttl_seconds"] = int(1800)  # Shorter TTL for better memory usage
        ...
        defaults["telemetry_batch_size"] = int(min(500, cpu_count * 50))
        defaults["telemetry_flush_interval"] = 2.0  # More frequent flushing

        # Concurrency Optimization
        # Scale with CPU count but consider memory constraints
        if memory_gb >= 16:
            defaults["max_concurrent_executions"] = int(cpu_count * 4)
        elif memory_gb >= 8:
            defaults["max_concurrent_executions"] = int(cpu_count * 3)
        else:
            defaults["max_concurrent_executions"] = int(cpu_count * 2)

        defaults["concurrency_adaptation_rate"] = 0.15  # Moderate adaptation

        # Cache Optimization
        defaults["cache_max_size"] = int(1000 * memory_factor)
        defaults["cache_ttl_seconds"] = int(1800)  # Shorter TTL
    ...
    async def benchmark_configuration(
        ...
        try:
            ...
            return {"performance": avg_time, "memory": avg_memory, "stability": stability}
    ...
    async def tune_parameter_category(
        ...
        category_params = {
            name: spec for name, spec in self.parameters.items() if spec.category == category
        }
        ...
        for iteration in range(max_iterations):
            ...
            for name, spec in category_params.items():
                ...
                for new_value in spec.get_next_values():
                    ...
                    perf_improvement = (
                        baseline_metrics["performance"] - test_metrics["performance"]
                    ) / baseline_metrics["performance"]
                    memory_improvement = (
                        baseline_metrics["memory"] - test_metrics["memory"]
                    ) / max(baseline_metrics["memory"], 0.001)

                    # Combined score (weighted)
                    combined_improvement = perf_improvement * 0.7 + memory_improvement * 0.3

                    if (
                        combined_improvement > best_improvement
                        and test_metrics["stability"] >= self.stability_threshold
                    ):
            ...
            if best_param and best_improvement >= self.performance_threshold:
                ...
                result = TuningResult(
                    ...
                    memory_improvement=(baseline_metrics["memory"] - metrics["memory"])
                    / max(baseline_metrics["memory"], 0.001),
    ...
    def get_tuning_summary(self) -> JSONObject:
        ...
        with self._lock:
            ...
            categories_tuned = len(
                set(result.parameter_name.split("_")[0] for result in self.tuning_history)
            )
            ...
            for result in self.tuning_history:
                ...
                if (
                    category not in category_improvements
                    or result.performance_improvement > category_improvements[category]
                ):
            ...
            return {
                ...
                "tuning_history": [
                    result.to_dict() for result in self.tuning_history[-10:]
                ],  # Last 10 results
            }
    ...
    async def tune_all_categories(
        ...
        for category_name, objective_func in objective_functions.items():
            print(f"Tuning {category_name} parameters...")
            parameter_ranges.get(category_name, {})
            results = await self.tune_parameter_category(
                ParameterCategory(category_name.replace("_", " ").title().replace(" ", "")),
                objective_func,
                max_iterations_per_category,
            )
            ...
            if results:
                ...
                print(f"  No improvements found for {category_name}")
