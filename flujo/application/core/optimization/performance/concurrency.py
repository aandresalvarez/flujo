...
from ..memory.memory_utils import get_global_memory_optimizer
...
class AdaptiveSemaphore:
    ...
    def _determine_load_level(self, avg_acquisition_time: float, contention_rate: float) -> LoadLevel:
    ...
    async def _adjust_limit(self, new_limit: int) -> None:
        ...
        if new_limit > self.current_limit:
            ...
            for _ in range(permits_to_acquire):
                ...
                except asyncio.exceptions.WouldBlock:
                    # Can't acquire more permits, limit reached naturally
                    break
...
class WorkStealingQueue:
    ...
    def __init__(
        ...
        self._worker_queues: List[deque] = [deque() for _ in range(self.num_workers)]
        ...
        if self.enable_stats:
            ...
            if work_item.execution_time_seconds is not None:
                ...
                self._stats.max_execution_time_ns = max(
                    self._stats.max_execution_time_ns, 
                    execution_time_ns
                )
    ...
    def _steal_work(self, worker_id: int) -> Optional[WorkItem]:
        ...
        # Try to steal from other workers (avoid own queue)
        for i in range(self.num_workers):
            if i == worker_id:
                continue
            
            # Try to steal from the end of the queue (LIFO for better cache locality)
            with self._worker_locks[i]:
                queue = self._worker_queues[i]
                if len(queue) > 1:  # Only steal if queue has multiple items
                    work_item = queue.pop()  # Take from end
                    self._successful_steals += 1
                    
                    if self.enable_stats:
                        self._stats.work_steals += 1
    ...
    def get_stats(self) -> Optional[ConcurrencyStats]:
        """Get concurrency statistics."""
        if self._stats:
            # Update current queue count
            self._stats.queued_tasks = self.get_total_queued_work()
        return self._stats
...
class ConcurrencyOptimizations:
    ...
    def __init__(
        ...
        self._semaphore = AdaptiveSemaphore(
            initial_limit=self.initial_concurrency,
            enable_stats=enable_stats
        ) if enable_adaptive_limits else asyncio.Semaphore(self.initial_concurrency)
        
        self._work_queue = WorkStealingQueue(
            enable_work_stealing=enable_work_stealing,
            enable_stats=enable_stats
        ) if enable_work_stealing else None
        ...
        if self._work_queue:
            ...
            for worker_id in range(self._work_queue.num_workers):
                worker_task = asyncio.create_task(
                    ...
                    name=f"concurrency_worker_{worker_id}"
                )
    ...
    async def _execute_with_semaphore(self, coro: Awaitable[Any]) -> Any:
        ...
        try:
            ...
            # Update statistics
            if self.enable_stats and self._global_stats:
                ...
                self._global_stats.max_execution_time_ns = max(
                    self._global_stats.max_execution_time_ns,
                    execution_time
                )
    ...
    def get_concurrency_stats(self) -> Dict[str, Any]:
        ...
        if isinstance(self._semaphore, AdaptiveSemaphore):
            ...
            if semaphore_stats:
                stats['semaphore'] = {
                    'current_limit': self._semaphore.get_current_limit(),
                    'contentions': semaphore_stats.semaphore_contentions,
                    'avg_wait_time_ns': semaphore_stats.average_wait_time_ns
                }
        
        # Work queue stats
        if self._work_queue:
            queue_stats = self._work_queue.get_stats()
            if queue_stats:
                stats['work_queue'] = {
                    'total_tasks': queue_stats.total_tasks,
                    'completed_tasks': queue_stats.completed_tasks,
                    'queued_tasks': queue_stats.queued_tasks,
                    'work_steals': queue_stats.work_steals,
                    'completion_rate': queue_stats.completion_rate
                }
            
            # Queue sizes
            stats['queue_sizes'] = self._work_queue.get_queue_sizes()
            
            # Steal stats
            steal_attempts, successful_steals = self._work_queue.get_steal_stats()
            stats['steal_stats'] = {
                'attempts': steal_attempts,
                'successful': successful_steals,
                'success_rate': successful_steals / max(steal_attempts, 1)
            }
        ...
        if self._global_stats:
            stats['global'] = {
                ...
                'completion_rate': self._global_stats.completion_rate,
                'avg_execution_time_ns': self._global_stats.average_execution_time_ns
            }
...
_global_concurrency_optimizer: Optional[ConcurrencyOptimizations] = None


def get_global_concurrency_optimizer() -> ConcurrencyOptimizations:
    """Get the global concurrency optimizer instance."""
    global _global_concurrency_optimizer
    if _global_concurrency_optimizer is None:
        _global_concurrency_optimizer = ConcurrencyOptimizations()
...
async def execute_with_optimized_concurrency(
    ...
    """Convenience function to execute coroutine with concurrency optimization."""
...
def get_concurrency_stats() -> Dict[str, Any]:
    """Convenience function to get concurrency statistics."""
    optimizer = get_global_concurrency_optimizer()
