...
class AdaptiveSemaphore:
    ...
    def __init__(
        ...
        self._execution_times: deque[int] = deque(maxlen=100)
        self._wait_times: deque[int] = deque(maxlen=100)
        self._queue_sizes: deque[int] = deque(maxlen=100)
    ...
    async def acquire(self) -> None:
        ...
        with self._lock:
            self._execution_times.append(acquisition_time)
    ...
    async def _maybe_adapt(self) -> None:
        ...
        with self._lock:
            ...
            avg_acquisition_time = sum(self._execution_times) / len(self._execution_times)
...
class WorkStealingQueue:
    ...
    def submit_work(self, work_item: WorkItem) -> None:
        ...
        with self._worker_locks[worker_id]:
            self._worker_queues[worker_id].append(work_item)
    ...
    def complete_work(self, work_item: WorkItem, success: bool = True) -> None:
        ...
        if self.enable_stats and self._stats is not None:
            ...
            if work_item.started_at is not None:
                execution_time = work_item.completed_at - work_item.started_at
                self._stats.total_execution_time_ns += int(execution_time * 1_000_000_000)
                self._stats.max_execution_time_ns = max(
                    self._stats.max_execution_time_ns, int(execution_time * 1_000_000_000)
                )
    ...
    def _steal_work(self, worker_id: int) -> Optional[WorkItem]:
        ...
        for i in range(1, self.num_workers):
            target_worker = (worker_id + i) % self.num_workers
            work_item = self._get_work_from_queue(target_worker)
...
class ConcurrencyOptimizations:
    ...
    async def __aenter__(self) -> "ConcurrencyOptimizations":
