...
from typing import Any, Dict, List, Type, TypeVar, Generic, Optional, Set
...
class OptimizedObjectPool:
    ...
    # Internal state
    _pools: Dict[Type, deque] = field(default_factory=dict, init=False)
    _locks: Dict[Type, asyncio.Lock] = field(default_factory=dict, init=False)
    _stats: Dict[Type, PoolStats] = field(default_factory=dict, init=False)
    _weak_refs: Set[weakref.ref] = field(default_factory=set, init=False)
    ...
    async def get(self, obj_type: Type[T], *args, **kwargs) -> T:
        """
    ...
    async def clear(self, obj_type: Optional[Type] = None) -> None:
        """
        Clear pool(s).
    ...
    def get_stats(self, obj_type: Optional[Type] = None) -> Dict[Type, PoolStats]:
        """
    ...
    async def _get_pool(self, obj_type: Type) -> deque:
    ...
    async def _get_lock(self, obj_type: Type) -> asyncio.Lock:
    ...
    def _get_stats(self, obj_type: Type) -> PoolStats:
    ...
    def _update_stats(self, obj_type: Type, stat_type: str) -> None:
    ...
    async def _create_object(self, obj_type: Type[T], *args, **kwargs) -> T:
        ...
        try:
            ...
            if hasattr(obj_type, '__call__'):
                if asyncio.iscoroutinefunction(obj_type):
                    return await obj_type(*args, **kwargs)
                else:
    ...
    def _create_builtin(self, obj_type: Type[T], *args, **kwargs) -> T:
        """Create builtin type instances (fast path)."""
        self._update_stats(obj_type, 'created_object')
        
        if obj_type == str:
            return str(*args, **kwargs) if args or kwargs else ""
        elif obj_type == int:
            return int(*args, **kwargs) if args or kwargs else 0
        elif obj_type == float:
            return float(*args, **kwargs) if args or kwargs else 0.0
        elif obj_type == bool:
            return bool(*args, **kwargs) if args or kwargs else False
        elif obj_type == list:
            return list(*args, **kwargs) if args or kwargs else []
        elif obj_type == dict:
            return dict(*args, **kwargs) if args or kwargs else {}
        elif obj_type == set:
            return set(*args, **kwargs) if args or kwargs else set()
        elif obj_type == tuple:
            return tuple(*args, **kwargs) if args or kwargs else ()
        else:
            return obj_type(*args, **kwargs)
...
class TypedObjectPool(Generic[T]):
    """
    Type-specific object pool for better type safety.
    
    This is a wrapper around OptimizedObjectPool that provides
    type-specific operations for a single object type.
    """
    ...
    async def get(self, *args, **kwargs) -> T:
        """Get object of the specific type."""
    ...
    def get_stats(self) -> PoolStats:
        ...
        return stats.get(self.obj_type, PoolStats())
...
def get_global_pool() -> OptimizedObjectPool:
    """Get the global object pool instance."""
    global _global_pool
...
async def get_pooled_object(obj_type: Type[T], *args, **kwargs) -> T:
    """Convenience function to get object from global pool."""
    pool = get_global_pool()
    return await pool.get(obj_type, *args, **kwargs)


async def return_pooled_object(obj: Any) -> None:
    """Convenience function to return object to global pool."""
    pool = get_global_pool()
    await pool.put(obj)


def create_typed_pool(obj_type: Type[T], **pool_kwargs) -> TypedObjectPool[T]:
    """Create a typed object pool for a specific type."""
...
class PooledObject(Generic[T]):
    ...
    def __init__(self, obj_type: Type[T], pool: Optional[OptimizedObjectPool] = None, *args, **kwargs):
        self.obj_type = obj_type
