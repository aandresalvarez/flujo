...
from typing import Any, Dict, List, Optional, Set, Callable, Union, ContextManager, AsyncContextManager
...
class BatchProcessor:
    ...
    async def _process_batches(self, force: bool = False) -> None:
        ...
        should_flush = (
            ...
            self._span_buffer.current_size() >= self.batch_size or
            self._metric_buffer.current_size() >= self.batch_size
        )
...
class OptimizedTelemetry:
    ...
    def __init__(
        ...
        self._span_pool: deque = deque(maxlen=1000)
        self._metric_pool: deque = deque(maxlen=1000)
    ...
    @contextmanager
    def trace(self, name: str, tags: Optional[Dict[str, Any]] = None) -> ContextManager[Optional[Span]]:
    ...
    @asynccontextmanager
    async def trace_async(self, name: str, tags: Optional[Dict[str, Any]] = None) -> AsyncContextManager[Optional[Span]]:
    ...
    def record_metric(
        ...
        try:
            ...
            if self.enable_stats:
                ...
                self._stats.max_metric_overhead_ns = max(self._stats.max_metric_overhead_ns, overhead)
    ...
    def set_gauge(self, name: str, value: Union[int, float], tags: Optional[Dict[str, Any]] = None) -> None:
        """Set a gauge metric."""
        self.record_metric(name, value, MetricType.GAUGE, tags)
    
    def record_histogram(self, name: str, value: Union[int, float], tags: Optional[Dict[str, Any]] = None) -> None:
        """Record a histogram metric."""
        self.record_metric(name, value, MetricType.HISTOGRAM, tags)
    
    def record_timer(self, name: str, duration_ns: int, tags: Optional[Dict[str, Any]] = None) -> None:
    ...
    def _get_span_from_pool(self) -> Span:
        ...
        if self._span_pool:
            ...
            track_object_creation(span, "telemetry_span")
    ...
    def _return_span_to_pool(self, span: Span) -> None:
        """Return span to object pool."""
        if len(self._span_pool) < self._span_pool.maxlen:
            span.reset()
            self._span_pool.append(span)
    
    def _get_metric_from_pool(self) -> Metric:
        """Get metric from object pool."""
        if self._metric_pool:
            ...
            track_object_creation(metric, "telemetry_metric")
    ...
    def _return_metric_to_pool(self, metric: Metric) -> None:
        """Return metric to object pool."""
        if len(self._metric_pool) < self._metric_pool.maxlen:
            metric.reset()
...
def trace_function(name: str, tags: Optional[Dict[str, Any]] = None):
    """Decorator for tracing functions."""
    def decorator(func: Callable) -> Callable:
        if asyncio.iscoroutinefunction(func):
            async def async_wrapper(*args, **kwargs):
                telemetry = get_global_telemetry()
                async with telemetry.trace_async(name, tags) as span:
                    if span:
            ...
            def sync_wrapper(*args, **kwargs):
                telemetry = get_global_telemetry()
                with telemetry.trace(name, tags) as span:
                    if span:
                        span.add_tag("function", func.__name__)
                        span.add_tag("module", func.__module__)
...
def finish_span(span: Optional[Span], status: str = "ok") -> None:
    """Convenience function to finish a span."""
    telemetry = get_global_telemetry()
