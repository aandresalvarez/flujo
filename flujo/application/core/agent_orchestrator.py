...
class AgentOrchestrator:
    ...
    async def execute(
        ...
        *,
        ...
        from ...domain.models import Failure, Success
        ...
        if fallback_depth > 0:
            try:
                if core._fallback_handler.is_step_in_chain(step):
                    try:
                        core._fallback_handler.reset()
                    except Exception:
                        pass
                core._fallback_handler.push_to_chain(step)
            except Exception:
                pass
        ...
        if fallback_depth > 0:
            total_attempts = 1
        telemetry.logfire.debug(
            f"[AgentOrchestrator] SimpleStep max_retries (total attempts): {total_attempts}"
        )
            ...
            try:
                ...
                if hasattr(step, "plugins") and step.plugins:
                    ...
                    try:
                        ...
                        try:
                            ...
                            if (
                                getattr(fb_exc, "__class__", type(fb_exc)).__name__
                                == "InfiniteFallbackError"
                            ):
                                fb_txt = f"Fallback loop detected for step '{getattr(fb_step, 'name', '<unnamed>')}'"
                                return Failure(
                                    error=fb_exc,
                                    feedback=fb_txt,
                        ...
                        combo = f"Original error: {result.feedback}; Fallback error: {fallback_result_sr.feedback}"
                        try:
                            fb_tokens = (
                            ...
                            try:
                                ...
                                fb_tokens = 0
                        try:
                            if best_primary_tokens:
                                primary_tokens = best_primary_tokens * max(1, total_attempts)
                            elif primary_tokens_total:
                                primary_tokens = primary_tokens_total * max(1, total_attempts)
                            else:
                        ...
                        if fallback_result_sr.success:
                            ...
                            try:
                                ...
                                pass
                            try:
                                unit_primary = int(best_primary_tokens)
                            except Exception:
                                unit_primary = 0
                            if unit_primary == 0:
                                try:
                                    unit_primary = int(primary_tokens_total) // max(
                                        1, total_attempts
                                    )
                                except Exception:
                                    pass
                            ...
                            primary_total_tokens = (
                                ...
                                if primary_tokens_total not in (None, 0)
                            ...
                            try:
                                fallback_result_sr.token_counts = int(
                ...
                if fb_step is not None:
                    try:
                        try:
                            telemetry.logfire.debug(
                                f"[AgentOrchestrator] Invoking fallback step '{getattr(fb_step, 'name', '<unnamed>')}' after exception for '{getattr(step, 'name', '<unnamed>')}' attempt={attempt}/{total_attempts}"
                            )
                        except Exception:
                            pass
                        try:
                    ...
                    if isinstance(fallback_result_sr, StepResult):
                        if fallback_result_sr.success:
                            ...
                            if primary_unit_tokens == 0:
                                try:
                                    primary_unit_tokens = int(best_primary_tokens or 0)
                                except Exception:
                                    primary_unit_tokens = 0
                            if primary_unit_tokens == 0:
                                try:
                                    primary_unit_tokens = int(primary_tokens_total or 0) // max(
                                        1, total_attempts
                                    )
                                except Exception:
                                    primary_unit_tokens = 0
                            ...
                            fb_unit = (
                                fb_tokens if fb_tokens not in (None, 0) else primary_unit_tokens
                            )
                            ...
                            try:
                                await core._usage_meter.add(
                                    float(fallback_result_sr.cost_usd or 0.0),
                                    int(prompt_tokens_latest or 0) + fb_tokens,
                                    0,
                                )
                            except Exception:
                                pass
                            ...
                            exit_cm = None
                            return Success(step_result=fallback_result_sr)
                        primary_fb_failure = last_plugin_failure_feedback or primary_fb
                    ...
                    elif isinstance(fallback_result_sr, _StepOutcome):
                        if (
                            ...
                            return Success(step_result=fallback_result_sr.step_result)
                        sr_fb = core._unwrap_outcome_to_step_result(
                            fallback_result_sr, core._safe_step_name(step)
                        )
                        failure_fb = last_plugin_failure_feedback or primary_fb
                        return Failure(
                            error=e,
                ...
                return Failure(
                    ...
                    step_result=StepResult(
                        ...
                        attempts=result.attempts,
                        latency_s=time_perf_ns_to_seconds(time_perf_ns() - start_ns),
                        token_counts=best_primary_tokens or result.token_counts,
                        cost_usd=best_primary_cost_usd or result.cost_usd,
                        feedback=primary_fb,
                        branch_context=(
                            pre_attempt_context
                            if getattr(step, "updates_context", False)
        ...
        return Failure(
            error=Exception(
                f"[AgentOrchestrator] Unexpected fallthrough for step='{getattr(step, 'name', '<unnamed>')}'"
            ),
    ...
    def guard_fallback_loop(self, core: Any, step: Any, depth: int) -> None:
        ...
        try:
            ...
            if depth > 0 and handler.is_step_in_chain(step):
                # Do not raise; let caller surface a graceful failure
                handler.reset()
                return
            if depth > 0:
