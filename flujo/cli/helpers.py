"""CLI helper functions for pipeline operations and agent management."""
...
from flujo.infra.config_manager import get_cli_defaults as _get_cli_defaults_direct
from flujo.recipes.factories import make_default_pipeline
from flujo.utils.serialization import safe_deserialize, safe_serialize, serialize_to_json_robust
...
def parse_context_data(
    ...
    if context_data:
        try:
            ...
            return safe_deserialize(json.loads(context_data))
        ...
        try:
            with open(context_file, "r") as f:
                if context_file.endswith((".yaml", ".yml")):
                    ...
                    from flujo.cli.main import safe_deserialize
                    return safe_deserialize(json.load(f))
        except Exception:
            raise Exit(1)
...
def validate_context_model(
    ...
    try:
        ...
        return cast(Type[PipelineContext], context_model_class)
    except Exit:
        raise
    except Exception as e:
        from typer import secho
...
def run_solve_pipeline(
    ...
    from flujo.cli.main import make_default_pipeline
...
def create_improvement_report_table(
    suggestions: List[Any]
) -> tuple[Dict[str, List[Any]], Table]:
...
def create_pipeline_results_table(step_history: List[Any]) -> Table:
    ...
    def add_rows(step_res: Any, prefix: str = "") -> None:
        ...
        output = str(step_res.output)[:100] + "..." if len(str(step_res.output)) > 100 else str(step_res.output)
...
) -> tuple[Any, str, Any, Optional[Dict[str, Any]], Optional[Type]]:
...
def create_flujo_runner(
    pipeline: Any,
    context_model_class: Optional[Type],
...
def get_masked_settings_dict() -> Dict[str, Any]:
    ...
    settings = load_settings()
    return settings.model_dump(exclude={"openai_api_key", "logfire_api_key"})
...
def load_mermaid_code(
    file: str, object_name: str, detail_level: str
) -> str:
...
def validate_pipeline_file(path: str):
