...
import os
from typing import ClassVar, Dict, Literal, Optional, List
...
class Settings(BaseSettings):
    ...
    openai_api_key: Optional[SecretStr] = Field(
        None, validation_alias=AliasChoices("OPENAI_API_KEY", "orch_openai_api_key")
    )
    google_api_key: Optional[SecretStr] = Field(
        None, validation_alias=AliasChoices("GOOGLE_API_KEY", "orch_google_api_key")
    )
    anthropic_api_key: Optional[SecretStr] = Field(
        None, validation_alias=AliasChoices("ANTHROPIC_API_KEY", "orch_anthropic_api_key")
    )
    ...
    def load_dynamic_api_keys(self) -> "Settings":
        ...
        for key, value in os.environ.items():
            if key.upper().endswith("_API_KEY") and key.upper() not in handled_keys:
                provider_name = key[:-8].lower()
                if value:
