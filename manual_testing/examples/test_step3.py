    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        if result and result.final_pipeline_context:
            ...
            if final_context.is_clear:
                print(f"  ✅ SUCCESS: Definition was clarified")
            else:
                print(f"  ⚠️ WARNING: Definition was not clarified (may have hit max_loops)")
                
            if final_context.clarification_count >= test_case['expected_clarifications']:
                print(f"  ✅ SUCCESS: Expected number of clarifications or more")
            else:
                print(f"  ⚠️ NOTE: Fewer clarifications than expected")
