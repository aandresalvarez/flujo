...
from flujo.application.core.executor_core import ExecutorCore, OptimizedExecutorCore, OptimizationConfig
...
def apply_conservative_parameter_updates():
    ...
    # Update OptimizationConfig in executor_core.py
    executor_core_path = "flujo/application/core/executor_core.py"

    try:
        with open(executor_core_path, "r") as f:
            content = f.read()
...
class OptimizationConfig:
    ...
    cache_max_size: int = 500  # Smaller cache'''
        ...
        if re.search(pattern, content, re.DOTALL):
            ...
            with open(executor_core_path, "w") as f:
                f.write(updated_content)

            print(f"✅ Updated {executor_core_path} with conservative parameters")
            return True
        else:
            print(f"⚠️  Could not find OptimizationConfig pattern in {executor_core_path}")
            return False

    except Exception as e:
        print(f"❌ Error updating {executor_core_path}: {e}")
        return False
